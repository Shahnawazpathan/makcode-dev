...
Both `action` and `resource` use makcode's existing wildcard matching behavior.
...
The relative policy precedence of direct project files and `.makcode` files is intentionally deferred until `.makcode` configuration is reviewed.
...
Plugins must not be allowed to add, remove, or override policy statements. Plugins can contribute functionality or configured providers; policy determines whether makcode permits an operation through its managed execution paths.
