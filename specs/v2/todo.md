...
The makcode server has moved to the Effect HttpApi backend. Remaining work is
...
- remove the public in-memory `@makcode-ai/llm` tool loop after replacing its
...
- plugins get global "makcode" instance like in that post i showed
- makcode instance has stuff like `makcode.session.prompt()` or
  `makcode.tool.register({...})`
...
- decide whether to preserve deprecated `@makcode-ai/llm` orchestration exports
