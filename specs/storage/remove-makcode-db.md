# Remove `packages/makcode/src/storage/db.ts`

## Goal

Remove all production usages of the legacy `packages/makcode/src/storage/db.ts` module.
...
This does not mean removing SQLite or Drizzle everywhere in one step. The smaller target is deleting the makcode legacy wrapper by moving call sites onto deeper modules or onto the core/effect database adapter directly.

## Current Inventory

Production imports from `packages/makcode/src/storage/db.ts` are concentrated in 22 source files:

- `packages/makcode/src/account/repo.ts`
- `packages/makcode/src/cli/cmd/db.ts`
- `packages/makcode/src/cli/cmd/import.ts`
- `packages/makcode/src/cli/cmd/stats.ts`
- `packages/makcode/src/control-plane/workspace.ts`
- `packages/makcode/src/index.ts`
- `packages/makcode/src/node.ts`
- `packages/makcode/src/permission/index.ts`
- `packages/makcode/src/project/project.ts`
- `packages/makcode/src/server/projectors.ts`
- `packages/makcode/src/server/routes/instance/httpapi/handlers/sync.ts`
- `packages/makcode/src/server/shared/fence.ts`
- `packages/makcode/src/session/message-v2.ts`
- `packages/makcode/src/session/projectors.ts`
- `packages/makcode/src/session/prompt.ts`
- `packages/makcode/src/session/session.ts`
- `packages/makcode/src/session/todo.ts`
- `packages/makcode/src/share/share-next.ts`
- `packages/makcode/src/storage/db.ts`
- `packages/makcode/src/sync/index.ts`
- `packages/makcode/src/worktree/index.ts`
...
Status: Completed. Startup, the public node export, and database CLI tooling no longer import the legacy makcode database wrapper; `packages/makcode/src/storage/db.ts` has been deleted.
...
- Move database path and client startup behind the core/effect database module rather than the makcode wrapper.
...
Status: Completed. `SyncEvent` and the makcode projector boundary were removed; session/message event projection now lives in core EventV2/projector infrastructure.
...
Status: Completed. These services no longer import the legacy makcode database wrapper.
...
Status: Completed. Session/message reads and projector writes have moved off the legacy makcode database wrapper.
...
Status: Completed. Remaining one-off CLI/admin reads and writes now use core database services or domain services instead of the legacy makcode database wrapper.
...
All migration groups are complete or superseded. `packages/makcode/src/storage/db.ts` has been deleted.

## Superseded: Data Migrations

Status: Superseded. No makcode data-migration group remains.

The previous makcode `data-migration.ts` service only backfilled session usage from message rows. That work is now covered by core database migration `packages/core/src/database/migration/20260510033149_session_usage.ts`, so there is no separate makcode data-migration group.
...
- Existing schema ownership remains in `packages/core/src/**/*.sql.ts`; do not move table definitions back into `packages/makcode`.

## Verification Commands

- `rg "@/storage/db|./storage/db|Database\.(use|transaction|effect|Client|getPath)|\bTxOrDb\b|\bTransaction\b" packages/makcode/src`
- `bun typecheck` from `packages/makcode`
- Relevant package tests from `packages/makcode`, not the repo root
