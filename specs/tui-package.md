...
Move the canonical MakCode terminal application from
`packages/makcode/src/cli/cmd/tui` into a self-contained workspace package while
...
generic presentation libraries. It must not depend on `packages/makcode`,
`packages/cli`, or `@makcode-ai/core`.

The SDK is the TUI's MakCode boundary. Missing backend data or operations must
...
- Do not preserve private imports by creating aliases from `packages/tui` back
  into `packages/makcode`.
- Do not replace private `packages/makcode` imports with `@makcode-ai/core`
...
- Keep legacy CLI command parsing, server startup, worker management,
  authentication, and config discovery outside `@makcode-ai/tui`.
...
- MakCode domain data displayed by the TUI
...
```text
packages/makcode/src/cli/cmd/tui
```

Its private dependency on `packages/makcode` is primarily expressed through
the `@/*` TypeScript alias, which resolves to `packages/makcode/src/*`.
...
Status: Completed. The private `@makcode-ai/tui` workspace package now has an
...
- Add `packages/tui/package.json` with the name `@makcode-ai/tui`.
...
- The package has no dependency on `makcode`, `@makcode-ai/cli`, or
  `@makcode-ai/core`.
...
primitives now live in `@makcode-ai/tui`. The legacy host consumes explicit
...
and build inputs through `@makcode-ai/tui/runtime`. Movable app, component,
route, and feature-plugin code no longer reads MakCode globals or process state;
...
Replace process-global MakCode state with resolved TUI inputs.
...
`@makcode-ai/tui/config` now owns schemas, defaults, keybind resolution, the
...
- Keep config path discovery, project/global precedence, migration, variable
  expansion, and plugin package installation in `packages/makcode` initially.
...
canonical in `@makcode-ai/tui`. Configured references resolve through the new
...
adapters rather than MakCode domain implementations.

Make the SDK the only MakCode domain boundary used by the TUI.
...
- Domain-facing TUI code imports MakCode data and operations only from
  `@makcode-ai/sdk`.
...
`@makcode-ai/tui`. The legacy host injects a narrow plugin host that retains
...
- Move plugin rendering tests to `packages/tui`; retain installation/loading
  integration tests in `packages/makcode`.
...
- The package has no imports from `packages/makcode`, `packages/cli`, or
  `@makcode-ai/core`.
...
public `@makcode-ai/tui` root while retaining worker/server/config/plugin and
...
- Keep the legacy yargs commands corresponding to current `thread.ts` and
  `attach.ts` in `packages/makcode`.
- Keep the legacy embedded worker and server startup in `packages/makcode`.
- Change those adapters to load config, create transport inputs, and call the
  public `@makcode-ai/tui` API.
...
- Remove obsolete `@tui/*` path mappings from `packages/makcode`.
- Remove stale test fixtures and update all imports to package exports.
- Narrow `@makcode-ai/tui` exports to intentional public entrypoints.
...
- No source under `packages/tui` imports `@/...`, `@makcode-ai/core`, or either
  executable package.
...
- TUI package imports do not reach into executable or backend implementation
  packages.
- SDK wire data is treated as the source of truth for MakCode domain state.
...
cd packages/makcode && bun typecheck
...
rg '@makcode-ai/core|packages/makcode|packages/cli' packages/tui
rg 'src/cli/cmd/tui|@tui/' packages/makcode/src packages/makcode/test
```
...
- Build the current-platform `packages/makcode` binary.
