...
import { createOpencode } from "@makcode-ai/sdk"
...
`)
  ...
  const makcode = await createOpencode({ port: 0 })
  ...
  try {
    ...
    const session = await makcode.client.session.create()
    const result = await makcode.client.session
      .prompt({
