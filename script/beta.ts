...
const model = "makcode/gpt-5.3-codex"
...
async function build() {
  ...
  try {
    await $`./script/build.ts --single`.cwd("packages/makcode")
    return true
  } catch (err) {
...
async function fix(pr: PR, files: string[], prs: PR[], applied: number[], idx: number) {
  console.log(`  Trying to auto-resolve ${files.length} conflict(s) with makcode...`)
  ...
  try {
    await $`makcode run -m ${model} ${prompt}`
  } catch (err) {
    console.log(`  makcode failed: ${err}`)
    return false
  }
  ...
  console.log("  Conflicts resolved with makcode")
  return true
}
...
async function smoke(prs: PR[], applied: number[]) {
  ...
  console.log("\nTrying to fix final smoke check with makcode...")
  ...
  const prompt = [
    ...
    "Run `./script/build.ts --single` in `packages/makcode`.",
