---
title: ACP Support
description: Use MakCode in any ACP-compatible editor.
---

MakCode supports the [Agent Client Protocol](https://agentclientprotocol.com) or (ACP), allowing you to use it directly in compatible editors and IDEs.
...
To use MakCode via ACP, configure your editor to run the `opencode acp` command.

The command starts MakCode as an ACP-compatible subprocess that communicates with your editor over JSON-RPC via stdio.
...
To open it, use the new 'MakCode' agent in the AI Chat agent selector.
...
To use MakCode as an ACP agent in [CodeCompanion.nvim](https://github.com/olimorris/codecompanion.nvim), add the following to your Neovim config:
...
This config sets up CodeCompanion to use MakCode as the ACP agent for chat.
...
MakCode works the same via ACP as it does in the terminal. All features are supported:
...
- MCP servers configured in your MakCode config
