---
title: Provajderi
description: Korištenje bilo kojeg LLM provajdera u MakCode.
---

import config from "../../../../config.mjs"
export const console = config.console

MakCode koristi [AI SDK](https://ai-sdk.dev/) i [Models.dev](https://models.dev) za podršku **75+ LLM provajdera** i podržava pokretanje lokalnih modela.

Za dodavanje provajdera potrebno je:

1. Dodajte API ključeve za provajdera koristeći naredbu `/connect`.
2. Konfigurirajte dobavljača u vašoj MakCode konfiguraciji.
...
Možete prilagoditi dobavljače putem odjeljka `provider` u vašem MakCode
config.
...
MakCode Zen je lista modela koje je obezbedio MakCode tim koji su bili
testirano i potvrđeno da dobro radi sa MakCode. [Saznajte više](/docs/zen).

:::tip
Ako ste novi, preporučujemo da počnete sa MakCode Zen.
:::
...
Radi kao i svaki drugi provajder u MakCode i potpuno je opcionalan za korištenje.

---

## MakCode Go

MakCode Go je jeftin plan pretplate koji pruža pouzdan pristup popularnim modelima otvorenog kodiranja koje pruža MakCode tim i koji su testirani i verificirani da dobro rade s MakCode-om.

1. Pokrenite naredbu `/connect` u TUI-u, odaberite `MakCode Go` i idite na [opencode.ai/auth](https://opencode.ai/zen).
...
Da biste koristili Amazon Bedrock s MakCode:
...
:::info
[Anthropic] (https://anthropic.com) službeno ne podržava korištenje vaše Claude Pro/Max pretplate u MakCode.
:::
...
MakCode koristi mali model za neke AI zadatke kao što je generiranje naslova sesije.
Podrazumevano je konfigurisan da koristi gpt-5-nano, a hostuje ga Zen. Da zaključate MakCode
...
Za korištenje Google Vertex AI s MakCode:
...
Heliconeova funkcija [Sessions](https://docs.helicone.ai/features/sessions) vam omogućava da grupišete povezane LLM zahtjeve zajedno. Koristite dodatak [opencode-helicone-session](https://github.com/H2Shami/opencode-helicone-session) da automatski prijavite svaki MakCode razgovor kao sesiju u Helicone-u.
...
Dodatak ubacuje zaglavlja `Helicone-Session-Id` i `Helicone-Session-Name` u vaše zahtjeve. Na stranici Helicone Sessions, vidjet ćete svaki MakCode razgovor naveden kao zasebna sesija.
...
:::tip
Ollama se može automatski konfigurirati za MakCode. Pogledajte [Ollama integracijske dokumente](https://docs.ollama.com/integrations/opencode) za detalje.
:::
...
Da biste koristili Ollama Cloud s MakCode:
...
3. Kopirajte API ključ za korištenje u MakCode.
...
6. **Važno**: Prije upotrebe modela oblaka u MakCode, morate lokalno povući informacije o modelu:
...
MakCode Zen je lista testiranih i verifikovanih modela koju je obezbedio MakCode tim. [Saznajte više](/docs/zen).

1. Prijavite se na **<a href={console}>MakCode Zen</a>** i kliknite na **Kreiraj API ključ**.

2. Pokrenite naredbu `/connect` i potražite **MakCode Zen**.

```txt
   /connect
```

3. Unesite svoj MakCode API ključ.
...
Polja `limit` omogućavaju MakCode da shvati koliko vam je konteksta ostalo. Standardni dobavljači ih automatski preuzimaju sa models.dev.
...
2. Za prilagođene provajdere, provjerite MakCode konfiguraciju i:
