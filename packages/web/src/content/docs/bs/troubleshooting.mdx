...
Da biste otklonili probleme s MakCode, počnite provjeravanjem dnevnika i lokalnih podataka koje pohranjuje na disku.
...
MakCode pohranjuje podatke o sesiji i druge podatke aplikacije na disku na:
...
MakCode Desktop pokreće lokalni MakCode server (`opencode-cli` sidecar) u pozadini. Većina problema je uzrokovana nedostatkom dodatka, oštećenom keš memorijom ili lošim postavkama servera.
...
- samo za macOS: `MakCode` meni -> **Ponovo učitaj Webview** (pomaže ako je korisnički interfejs prazan/zamrznut).
...
MakCode također može učitati lokalne dodatke s diska. Privremeno ih maknite s puta (ili preimenujte folder) i ponovo pokrenite desktop aplikaciju:
...
Ako onemogućavanje dodataka ne pomogne (ili se instalacija dodatka zaglavila), obrišite keš memoriju kako bi ga MakCode mogao ponovo izgraditi.

1. Potpuno zatvorite MakCode Desktop.
...
3. Ponovo pokrenite MakCode Desktop.

---

### Rješavanje problema sa vezom na serveru

MakCode Desktop može ili pokrenuti svoj lokalni server (podrazumevano) ili se povezati na URL servera koji ste konfigurisali.
...
Na Windows-u, MakCode Desktop zahtijeva Microsoft Edge **WebView2 Runtime**. Ako se aplikacija otvori u praznom prozoru ili se ne pokrene, instalirajte/ažurirajte WebView2 i pokušajte ponovo.

---

### Windows: Opšti problemi sa performansama

Ako imate spore performanse, probleme s pristupom datotekama ili probleme s terminalom na Windows-u, pokušajte koristiti [WSL (Windows podsistem za Linux)](/docs/windows-wsl). WSL pruža Linux okruženje koje radi neprimetnije sa MakCode karakteristikama.

---

### Obavještenja se ne prikazuju

MakCode Desktop prikazuje sistemska obavještenja samo kada:

- obavještenja su omogućena za MakCode u postavkama vašeg OS-a, i
...
1. Zatvorite MakCode Desktop.
2. Pronađite i izbrišite ove datoteke (oni žive u direktoriju podataka MakCode Desktop aplikacije):
...
Ako naiđete na greške API poziva, to može biti zbog zastarjelih paketa dobavljača. MakCode dinamički instalira pakete dobavljača (OpenAI, Anthropic, Google, itd.) po potrebi i kešira ih lokalno.
...
2. Ponovo pokrenite MakCode da ponovo instalirate najnovije pakete dobavljača

Ovo će prisiliti MakCode da preuzme najnovije verzije paketa dobavljača, što često rješava probleme kompatibilnosti s parametrima modela i promjenama API-ja.
...
MakCode će otkriti da li koristite Wayland i preferirate `wl-clipboard`, u suprotnom će pokušati pronaći alate međuspremnika po redoslijedu: `xclip` i `xsel`.
