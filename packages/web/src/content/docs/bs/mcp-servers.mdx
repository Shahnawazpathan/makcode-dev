...
Možete dodati vanjske alate u MakCode koristeći _Model Context Protocol_, ili MCP. MakCode podržava i lokalne i udaljene servere.
...
Možete definirati MCP servere u vašoj [MakCode Config](https://opencode.ai/docs/config/) pod `mcp`. Dodajte svaki MCP sa jedinstvenim imenom. Možete se pozvati na taj MCP po imenu kada tražite LLM.
...
MakCode automatski rukuje OAuth autentifikacijom za udaljene MCP servere. Kada server zahtijeva autentifikaciju, MakCode će:
...
Ako server zahtijeva autentifikaciju, MakCode će vas tražiti prijavu pri prvom korištenju. Ako se to ne desi, možete [ručno pokrenuti tok](#authenticating) naredbom `opencode mcp auth <server-name>`.
...
Komanda `mcp auth` otvara pretraživač za autorizaciju. Nakon odobrenja, MakCode sigurno čuva tokene u `~/.local/share/opencode/mcp-auth.json`.
...
Vaši MCP serveri su dostupni kao alati u MakCode, zajedno s ugrađenim alatima. Možete njima upravljati kroz MakCode konfiguraciju kao i bilo kojim drugim alatom.
...
Ovo će otvoriti prozor pretraživača da završite OAuth tok i povežete MakCode sa vašim Sentry nalogom.
