...
MakCode Go est un abonnement à bas coût — **5 $ pour votre premier mois**, puis **10 $/mois** — qui vous donne un accès fiable aux modèles de codage ouverts populaires.

Go fonctionne comme n'importe quel autre fournisseur dans MakCode. Vous vous abonnez à MakCode Go et obtenez votre clé d'API. C'est **totalement facultatif** et vous n'avez pas besoin de l'utiliser pour utiliser MakCode.
...
MakCode Go vous donne accès à ces modèles pour **5 $ pour votre premier mois**, puis **10 $/mois**.

---

## Comment ça marche

MakCode Go fonctionne comme n'importe quel autre fournisseur dans MakCode.

1. Vous vous connectez à **<a href={console}>MakCode Zen</a>**, vous vous abonnez à Go et copiez votre clé d'API.
2. Vous exécutez la commande `/connect` dans la TUI, sélectionnez `MakCode Go` et collez votre clé d'API.
3. Exécutez `/models` dans la TUI pour voir la liste des modèles disponibles via Go.

:::note
Un seul membre par espace de travail peut s'abonner à MakCode Go.
:::
...
MakCode Go inclut les limites suivantes :
...
L'[ID de modèle](/docs/config/#models) dans votre configuration MakCode utilise le format `opencode-go/<model-id>`. Par exemple, pour Kimi K2.7 Code, vous utiliseriez `opencode-go/kimi-k2.7-code` dans votre configuration.
...
Nous avons créé MakCode Go pour :
...
4. N'avoir **aucun verrouillage exclusif** en vous permettant d'utiliser n'importe quel autre fournisseur avec MakCode également.
