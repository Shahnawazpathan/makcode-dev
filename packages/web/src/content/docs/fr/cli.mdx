---
title: CLI
description: Options et commandes de la CLI MakCode.
---

import { Tabs, TabItem } from "@astrojs/starlight/components"

La CLI MakCode démarre par défaut le [TUI](/docs/tui) lorsqu'il est exécuté sans aucun argument.

```bash
opencode
```

Mais elle accepte également les commandes documentées sur cette page. Cela vous permet d'interagir avec MakCode par programme.
...
Démarrez l’interface utilisateur du terminal MakCode.
...
La CLI MakCode dispose également des commandes suivantes.

---

### agent

Gérer les agents pour MakCode.
...
Attachez un terminal à un serveur backend MakCode déjà en cours d'exécution démarré via les commandes `serve` ou `web`.

```bash
opencode attach [url]
```

Cela permet d'utiliser le TUI avec un backend MakCode distant. Par exemple :
...
MakCode est alimenté par la liste des fournisseurs sur [Models.dev](https://models.dev), vous pouvez donc utiliser `opencode auth login` pour configurer les clés API pour tout fournisseur que vous souhaitez utiliser. Ceci est stocké dans `~/.local/share/opencode/auth.json`.

```bash
opencode auth login
```

Lorsque MakCode démarre, il charge les fournisseurs à partir du fichier d'informations d'identification, ainsi que les clés définies dans vos environnements ou un fichier `.env` dans votre projet.
...
Utilisez l'option `--refresh` pour mettre à jour la liste des modèles mis en cache. Ceci est utile lorsque de nouveaux modèles ont été ajoutés à un fournisseur et que vous souhaitez les voir dans MakCode.
...
Démarrez un serveur MakCode sans interface graphique pour un accès API. Consultez le [serveur docs](/docs/server) pour l'interface complète HTTP.

```bash
opencode serve
```

Cela démarre un serveur HTTP qui fournit un accès API aux fonctionnalités d'MakCode sans l'interface TUI. Définissez `OPENCODE_SERVER_PASSWORD` pour activer l'authentification de base HTTP (le nom d'utilisateur par défaut est `opencode`).
...
Liste toutes les sessions MakCode.
...
Affichez les statistiques d'utilisation et de coût des jetons pour vos sessions MakCode.
...
Importez les données de session à partir d'un fichier JSON ou d'une URL de partage MakCode.

```bash
opencode import <file>
```

Vous pouvez importer à partir d'un fichier local ou d'une URL de partage MakCode.
...
Démarrez un serveur MakCode sans interface graphique avec une interface Web.

```bash
opencode web
```

Cela démarre un serveur HTTP et ouvre un navigateur Web pour accéder à MakCode via une interface Web. Définissez `OPENCODE_SERVER_PASSWORD` pour activer l'authentification de base HTTP (le nom d'utilisateur par défaut est `opencode`).
...
Désinstallez MakCode et supprimez tous les fichiers associés.
...
MakCode peut être configuré à l'aide de variables d'environnement.
