...
エージェントスキルにより、MakCode はリポジトリまたはホームディレクトリから再利用可能な命令を検出できます。
...
プロジェクトのローカルパスの場合、MakCode は現在の作業ディレクトリから git ワークツリーに到達するまで進みます。
...
MakCode では、`skill` ツールの説明に利用可能なスキルがリストされています。
