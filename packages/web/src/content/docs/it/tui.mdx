---
title: TUI
description: Usa l'interfaccia utente da terminale (TUI) di MakCode.
---

import { Tabs, TabItem } from "@astrojs/starlight/components"

MakCode fornisce un'interfaccia interattiva da terminale (TUI) per lavorare sui tuoi progetti con un LLM.

Eseguire MakCode avvia la TUI per la directory corrente.
...
Quando usi la TUI di MakCode, puoi digitare `/` seguito dal nome di un comando per eseguire rapidamente azioni. Per esempio:
...
Aggiunge un provider a MakCode. Ti permette di selezionare tra i provider disponibili e aggiungere le relative chiavi API.
...
Esce da MakCode. _Alias_: `/quit`, `/q`
...
- `leader_timeout` - Controlla per quanto tempo MakCode attende dopo il leader key. Default: `2000`.
