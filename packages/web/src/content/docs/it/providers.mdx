---
title: Provider
description: Usa qualsiasi provider LLM in MakCode.
---

import config from "../../../../config.mjs"
export const console = config.console

MakCode usa [AI SDK](https://ai-sdk.dev/) e [Models.dev](https://models.dev) per supportare **oltre 75 provider LLM** e anche modelli locali.

Per aggiungere un provider devi:

1. Aggiungere le API key del provider con il comando `/connect`.
2. Configurare il provider nella configurazione di MakCode.
...
MakCode Zen e una lista di modelli forniti dal team MakCode, testati e
verificati per funzionare bene con MakCode. [Scopri di piu](/docs/zen).

:::tip
Se sei all'inizio, ti consigliamo di partire da MakCode Zen.
:::
...
Funziona come qualsiasi altro provider in MakCode ed e totalmente opzionale.
...
Per usare Amazon Bedrock con MakCode:
...
:::info
L'uso dell'abbonamento Claude Pro/Max in MakCode non è ufficialmente supportato da [Anthropic](https://anthropic.com).
:::
...
MakCode usa un modello piccolo per alcuni task AI come generare il titolo della sessione.
È configurato per usare gpt-5-nano di default, ospitato da Zen. Per bloccare MakCode
...
Per usare Google Vertex AI con MakCode:
...
La funzionalità [Sessions](https://docs.helicone.ai/features/sessions) di Helicone ti permette di raggruppare richieste LLM correlate. Usa il plugin [opencode-helicone-session](https://github.com/H2Shami/opencode-helicone-session) per loggare automaticamente ogni conversazione di MakCode come una sessione in Helicone.
...
Il plugin inietta gli header `Helicone-Session-Id` e `Helicone-Session-Name` nelle tue richieste. Nella pagina Sessions di Helicone, vedrai ogni conversazione di MakCode elencata come sessione separata.
...
:::tip
Ollama può auto-configurarsi per MakCode. Vedi la [documentazione di integrazione Ollama](https://docs.ollama.com/integrations/opencode) per dettagli.
:::
...
Per usare Ollama Cloud con MakCode:
...
3. Copia la chiave API da usare in MakCode.
...
6. **Importante**: Prima di usare modelli cloud in MakCode, devi scaricare le informazioni del modello localmente:
...
MakCode Zen è una lista di modelli testati e verificati forniti dal team MakCode. [Scopri di più](/docs/zen).

1. Accedi a **<a href={console}>MakCode Zen</a>** e clicca **Create API Key**.

2. Esegui il comando `/connect` e cerca **MakCode Zen**.

   ```txt
   /connect
   ```

3. Inserisci la tua chiave API di MakCode.
...
I campi `limit` permettono a MakCode di capire quanto contesto rimane. I provider standard recuperano questi dati da models.dev automaticamente.
