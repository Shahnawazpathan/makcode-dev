...
Per diagnosticare problemi con MakCode, inizia controllando i log e i dati locali che salva su disco.
...
MakCode Desktop esegue in background un server locale di MakCode (il sidecar `opencode-cli`). La maggior parte dei problemi e causata da un plugin che si comporta male, da una cache corrotta o da un'impostazione del server errata.
...
- Solo macOS: menu `MakCode` -> **Reload Webview** (utile se l'interfaccia e vuota o bloccata).
...
MakCode puo anche caricare plugin locali dal disco. Spostali temporaneamente altrove (o rinomina la cartella) e riavvia l'app desktop:
...
Se disabilitare i plugin non aiuta (o l'installazione di un plugin e bloccata), svuota la cache in modo che MakCode possa ricostruirla.

1. Chiudi completamente MakCode Desktop.
...
3. Riavvia MakCode Desktop.

---

### Risolvi problemi di connessione al server

MakCode Desktop puo avviare il proprio server locale (predefinito) oppure connettersi a un URL server che hai configurato.
...
Su Windows, MakCode Desktop richiede **WebView2 Runtime** di Microsoft Edge. Se l'app si apre su una finestra vuota o non parte, installa/aggiorna WebView2 e riprova.

---

### Windows: problemi generali di prestazioni

Se riscontri prestazioni lente, problemi di accesso ai file o problemi del terminale su Windows, prova a usare [WSL (Windows Subsystem for Linux)](/docs/windows-wsl). WSL fornisce un ambiente Linux che funziona in modo piu fluido con le funzionalita di MakCode.

---

### Notifiche non visualizzate

MakCode Desktop mostra le notifiche di sistema solo quando:

- le notifiche sono abilitate per MakCode nelle impostazioni del sistema operativo, e
...
1. Chiudi MakCode Desktop.
2. Trova ed elimina questi file (si trovano nella directory dati dell'app MakCode Desktop):
...
Se riscontri problemi con MakCode:
