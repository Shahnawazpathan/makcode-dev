...
MakCode gir et interaktivt terminalgrensesnitt (TUI) for å jobbe med prosjektene dine med en LLM.

Å kjøre MakCode starter TUI for gjeldende katalog.
...
Når du bruker MakCode TUI, kan du skrive `/` etterfulgt av et kommandonavn for raskt å utføre handlinger. For eksempel:
...
Legg til en leverandør til MakCode. Lar deg velge fra tilgjengelige leverandører og legge til deres API-nøkler.
...
Avslutt MakCode. _Aliaser_: `/quit`, `/q`
...
- `leader_timeout` - Styrer hvor lenge MakCode venter etter leader key. Standard er `2000`.
