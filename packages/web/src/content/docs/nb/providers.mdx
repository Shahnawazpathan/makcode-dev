---
title: Leverandører
description: Bruke en hvilken som helst LLM-leverandør i MakCode.
---

import config from "../../../../config.mjs"
export const console = config.console

MakCode bruker [AI SDK](https://ai-sdk.dev/) og [Models.dev](https://models.dev) for å støtte **75+ LLM leverandører** og den støtter kjøring av lokale modeller.

For å legge til en leverandør må du:

1. Legg til API-nøkkelen for leverandøren ved å bruke kommandoen `/connect`.
2. Konfigurer leverandøren i MakCode-konfigurasjonen.
...
Du kan tilpasse leverandørene gjennom `provider`-delen i MakCode
konfig.
...
MakCode Zen er en liste over modeller levert av MakCode-teamet som har vært
testet og verifisert for å fungere godt med MakCode. [Finn ut mer](/docs/zen).

:::tip
Hvis du er ny, anbefaler vi å starte med MakCode Zen.
:::

1. Kjør kommandoen `/connect` i TUI, velg `MakCode Zen` og gå til [opencode.ai/auth](https://opencode.ai/zen).
...
Det fungerer som alle andre leverandører i MakCode og er helt valgfritt å bruke.

---

## MakCode Go

MakCode Go er en lavpris abonnementsplan som gir pålitelig tilgang til populære åpne kodemodeller levert av MakCode-teamet som har vært
testet og verifisert for å fungere godt med MakCode.

1. Kjør kommandoen `/connect` i TUI, velg `MakCode Go`, og gå til [opencode.ai/auth](https://opencode.ai/zen).
...
Slik bruker du Amazon Bedrock med MakCode:
...
2. **Konfigurer autentisering** ved å bruke en av følgende metoder:
   ...
   Angi en av disse miljøvariablene mens du kjører MakCode:
...
2. Gå til [Azure AI Foundry](https://ai.azure.com/) og distribuer en modell.
   ...
   :::note
   Distribusjonsnavnet må samsvare med modellnavnet for at MakCode skal fungere skikkelig.
   :::
...
5. Kjør kommandoen `/models` for å velge en modell.
   ...
   Du kan også legge til modeller gjennom MakCode-konfigurasjonen.
...
for å lagre token i MakCode auth-lagring.
:::

##### Selvhostet GitLab

:::note[Merknad om samsvar]
MakCode bruker en liten modell for noen AI oppgaver som å generere økttittelen.
Den er konfigurert til å bruke gpt-5-nano som standard, hostet av Zen. For å låse MakCode
...
Slik bruker du GitHub Copilot-abonnementet med MakCode:
...
Slik bruker du Google Vertex AI med MakCode:
...
2. Angi de nødvendige miljøvariablene:
   ...
   Sett dem mens du kjører MakCode.
...
I tilfelle du ser en funksjon eller modell fra Helicone som ikke konfigureres automatisk gjennom MakCode, kan du alltid konfigurere den selv.
...
Helicones [Sessions](https://docs.helicone.ai/features/sessions) funksjon lar deg gruppere relaterte LLM forespørsler sammen. Bruk [opencode-helicone-session](https://github.com/H2Shami/opencode-helicone-session) plugin for automatisk å logge hver MakCode-samtale som en økt i Helicone.
...
Programtillegget injiserer `Helicone-Session-Id` og `Helicone-Session-Name` headere i forespørslene dine. På Helicones Sessions-side vil du se hver MakCode-samtale oppført som en separat økt.
...
Du kan konfigurere MakCode for å bruke lokale modeller gjennom [llama.cpp](https://github.com/ggml-org/llama.cpp) llama-server-verktøyet.
...
Du kan konfigurere MakCode for å bruke lokale modeller gjennom LM Studio.
...
Du kan konfigurere MakCode for å bruke lokale modeller gjennom Ollama.

:::tip
Ollama kan automatisk konfigurere seg selv for MakCode. Se [Ollama-integrasjonsdokumentene](https://docs.ollama.com/integrations/opencode) for detaljer.
:::
...
Slik bruker du Ollama Cloud med MakCode:
...
3. Kopier API-nøkkelen for bruk i MakCode.
...
6. **Viktig**: Før du bruker skymodeller i MakCode, må du hente modellinformasjonen lokalt:
...
MakCode Zen er en liste over testede og verifiserte modeller levert av MakCode-teamet. [Finn ut mer](/docs/zen).

1. Logg på **<a href={console}>MakCode Zen</a>** og klikk på **Create API Key**.

2. Kjør kommandoen `/connect` og søk etter **MakCode Zen**.

   ```txt
   /connect
   ```

3. Skriv inn MakCode API-nøkkelen.
...
4. Mange OpenRouter-modeller er forhåndslastet som standard, kjør kommandoen `/models` for å velge den du ønsker.
   ...
   Du kan også legge til flere modeller gjennom MakCode-konfigurasjonen.
...
5. Du kan også tilpasse dem gjennom MakCode-konfigurasjonen. Her er et eksempel på å spesifisere en leverandør
...
Slik bruker du [Scaleway Generative APIs](https://www.scaleway.com/en/docs/generative-apis/) med MakCode:
...
Du kan også tilpasse modeller gjennom MakCode-konfigurasjonen. Her er et eksempel på spesifisering av leverandørrutingsrekkefølge.
...
:::tip
Du kan bruke hvilken som helst OpenAI-kompatibel leverandør med MakCode. De fleste moderne AI-leverandører tilbyr OpenAI-kompatible APIer.
:::
...
`limit`-feltene lar MakCode forstå hvor mye kontekst du har igjen. Standardleverandører henter disse automatisk fra models.dev.
...
2. For tilpassede leverandører, sjekk MakCode-konfigurasjonen og:
   - Sørg for at leverandør-ID-en som brukes i `/connect`-kommandoen samsvarer med ID i MakCode-konfigurasjonen.
