---
title: TUI
description: Using the MakCode terminal user interface.
---

import { Tabs, TabItem } from "@astrojs/starlight/components"

MakCode provides an interactive terminal interface or TUI for working on your projects with an LLM.

Running MakCode starts the TUI for the current directory.
...
When using the MakCode TUI, you can type `/` followed by a command name to quickly execute actions. For example:
...
Add a provider to MakCode. Allows you to select from available providers and add their API keys.
...
- `leader_timeout` - Controls how long MakCode waits after the leader key. Defaults to `2000`.
