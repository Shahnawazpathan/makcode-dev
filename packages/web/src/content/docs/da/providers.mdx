---
title: Udbydere
description: Brug en hvilken som helst LLM-udbyder i MakCode.
---

import config from "../../../../config.mjs"
export const console = config.console

MakCode bruger [AI SDK](https://ai-sdk.dev/) og [Models.dev](https://models.dev) til at understøtte **75+ LLM-udbydere** og understøtter kørsel af lokale modeller.

For at tilføje en udbyder skal du:

1. Tilføje API-nøglerne for udbyderen ved at bruge kommandoen `/connect`.
2. Konfigurere udbyderen i MakCode-konfigurationen.
...
Du kan tilpasse udbyderne gennem `provider`-delen i MakCode-konfigurationen.
...
MakCode Zen er en liste over modeller leveret af MakCode-teamet, som er blevet testet og verificeret til at fungere godt med MakCode. [Læs mere](/docs/zen).

:::tip
Hvis du er ny, anbefaler vi at starte med MakCode Zen.
:::
...
Det fungerer som alle andre udbydere i MakCode og er helt valgfrit at bruge.

---

## MakCode Go

MakCode Go er en billig abonnementsplan, der giver pålidelig adgang til populære åbne kodningsmodeller leveret af MakCode-teamet, som er testet og verificeret til at fungere godt med MakCode.

1. Kør kommandoen `/connect` i TUI, vælg `MakCode Go`, og gå til [opencode.ai/auth](https://opencode.ai/zen).
...
Sådan bruger du Amazon Bedrock med MakCode:
...
:::info
At bruge dit Claude Pro/Max-abonnement i MakCode understøttes ikke officielt af [Anthropic](https://anthropic.com).
:::
...
MakCode bruger en lille model til nogle AI-opgaver som at generere sessionstitlen.
Den er konfigureret til at bruge gpt-5-nano som standard, hostet af Zen. For at låse MakCode
...
Sådan bruger du GitHub Copilot-abonnementet med MakCode:
...
Sådan bruger du Google Vertex AI med MakCode:
...
Helicones [Sessions](https://docs.helicone.ai/features/sessions) funktion lader dig gruppe relaterede LLM anmodninger sammen. Brug [opencode-helicone-session](https://github.com/H2Shami/opencode-helicone-session) plugin for automatisk at logge hver MakCode-samtale som en session i Helicone.
...
Programmatisk injicerer `Helicone-Session-Id` og `Helicone-Session-Name` headers i dine anmodninger. På Helicones Sessions-side vil du se hver MakCode-samtale opført som en separat session.
...
:::tip
Ollama kan automatisk konfigurere sig selv for MakCode. Se [Ollama-integrationsdokumenterne](https://docs.ollama.com/integrations/opencode) for detaljer.
:::
...
Sådan bruger du Ollama Cloud med MakCode:
...
3. Kopier API-nøglen til brug i MakCode.
...
6. **Vigtigt**: Før du bruger skymodeller i MakCode, skal du hente modelinformationen lokalt:
...
MakCode Zen er en liste over testede og verificerede modeller leveret af MakCode-teamet. [Læs mere](/docs/zen).

1. Log på **<a href={console}>MakCode Zen</a>** og klik på **Create API Key**.

2. Kør kommandoen `/connect` og søg efter **MakCode Zen**.

   ```txt
   /connect
   ```

3. Indtast MakCode API-nøglen.
...
Sådan bruger du [Scaleway Generative APIs](https://www.scaleway.com/en/docs/generative-apis/) med MakCode:
...
`limit`-felterne lader MakCode forstå hvor meget kontekst du har tilbage. Standardudbydere henter disse automatisk fra models.dev.
