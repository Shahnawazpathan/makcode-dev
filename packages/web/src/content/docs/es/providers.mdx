---
title: Proveedores
description: Usando cualquier proveedor LLM en MakCode.
---

import config from "../../../../config.mjs"
export const console = config.console

MakCode usa el [AI SDK](https://ai-sdk.dev/) y [Models.dev](https://models.dev) para admitir **75+ proveedores LLM** y admite la ejecución de modelos locales.

Para agregar un proveedor necesita:

1. Agregue las claves API para el proveedor usando el comando `/connect`.
2. Configure el proveedor en su configuración MakCode.
...
Puedes personalizar los proveedores a través de la sección `provider` en tu MakCode
configuración.
...
MakCode Zen es una lista de modelos proporcionados por el equipo MakCode que han sido
probado y verificado para funcionar bien con MakCode. [Más información](/docs/zen).

:::tip
Si eres nuevo, te recomendamos comenzar con MakCode Zen.
:::
...
Funciona como cualquier otro proveedor en MakCode y su uso es completamente opcional.

---

## MakCode Go

MakCode Go es un plan de suscripción de bajo costo que brinda acceso confiable a modelos de codificación abiertos populares proporcionados por el equipo de MakCode que han sido
probado y verificado para funcionar bien con MakCode.

1. Ejecute el comando `/connect` en TUI, seleccione `MakCode Go` y diríjase a [opencode.ai/auth](https://opencode.ai/zen).
...
Para usar Amazon Bedrock con MakCode:
...
:::info
El uso de su suscripción Claude Pro/Max en MakCode no está oficialmente respaldado por [Anthropic](https://anthropic.com).
:::
...
MakCode utiliza un modelo pequeño para algunas tareas de IA, como generar el título de la sesión.
Está configurado para usar gpt-5-nano de forma predeterminada, alojado por Zen. Para bloquear MakCode
...
Para utilizar Google Vertex AI con MakCode:
...
La función [Sesiones](https://docs.helicone.ai/features/sessions) de Helicone le permite agrupar solicitudes LLM relacionadas. Utilice el complemento [opencode-helicone-session](https://github.com/H2Shami/opencode-helicone-session) para registrar automáticamente cada conversación MakCode como una sesión en Helicone.
...
El complemento inyecta encabezados `Helicone-Session-Id` y `Helicone-Session-Name` en sus solicitudes. En la página Sesiones de Helicone, verá cada conversación MakCode enumerada como una sesión separada.
...
:::tip
Ollama puede configurarse automáticamente para MakCode. Consulte los [documentos de integración de Ollama](https://docs.ollama.com/integrations/opencode) para obtener más detalles.
:::
...
Para usar Ollama Cloud con MakCode:
...
3. Copie la clave API para usarla en MakCode.
...
6. **Importante**: Antes de usar modelos en la nube en MakCode, debe obtener la información del modelo localmente:
...
MakCode Zen es una lista de modelos probados y verificados proporcionada por el equipo MakCode. [Más información](/docs/zen).

1. Inicie sesión en **<a href={console}>MakCode Zen</a>** y haga clic en **Crear API clave**.

2. Ejecute el comando `/connect` y busque **MakCode Zen**.

   ```txt
   /connect
   ```

3. Ingrese su clave MakCode API.
...
Para utilizar [API generativas de Scaleway](https://www.scaleway.com/en/docs/generative-apis/) con MakCode:
...
Los campos `limit` le permiten a MakCode comprender cuánto contexto le queda. Los proveedores estándar los extraen de models.dev automáticamente.
