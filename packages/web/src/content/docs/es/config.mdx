---
title: Configuración
description: Usando la configuración MakCode JSON.
---

Puede configurar MakCode usando un archivo de configuración JSON.

---

## Formato

MakCode admite los formatos **JSON** y **JSONC** (JSON con comentarios).
...
Coloque su configuración global MakCode en `~/.config/opencode/opencode.json`. Utilice la configuración global para las preferencias de todo el usuario, como temas, proveedores o combinaciones de teclas.
...
Cuando se inicia MakCode, busca un archivo de configuración en el directorio actual o recorre hasta el directorio Git más cercano.
...
- `mdns`: habilita el descubrimiento de servicios mDNS. Esto permite que otros dispositivos en la red descubran su servidor MakCode.
...
Puede configurar los proveedores y modelos que desea utilizar en su configuración MakCode a través de las opciones `provider`, `model` y `small_model`.
...
La opción `small_model` configura un modelo separado para tareas livianas como la generación de títulos. De forma predeterminada, MakCode intenta utilizar un modelo más económico si su proveedor tiene uno disponible; de ​​lo contrario, recurre a su modelo principal.
...
Puede configurar el tema que desea usar en su configuración MakCode a través de la opción `theme`.
...
El agente predeterminado debe ser un agente principal (no un subagente). Puede ser un agente integrado como `"build"` o `"plan"`, o un [agente personalizado](/docs/agents) que haya definido. Si el agente especificado no existe o es un subagente, MakCode recurrirá a `"build"` con una advertencia.
...
MakCode descargará automáticamente cualquier actualización nueva cuando se inicie. Puede desactivar esto con la opción `autoupdate`.
...
Los [complementos](/docs/plugins) amplían MakCode con herramientas, enlaces e integraciones personalizados.
...
Esto es útil cuando desea restringir MakCode para que utilice únicamente proveedores específicos en lugar de deshabilitarlos uno por uno.
