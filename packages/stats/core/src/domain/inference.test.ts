...
describe("inference stat normalization", () => {
  ...
  test("uses provider.model to resolve makcode route providers", () => {
    ...
    expect(statProvider("big-pickle", "claude-sonnet-4-5", "makcode")).toBe("anthropic")
    expect(statProvider("big-pickle", "gpt-5", "makcode")).toBe("openai")
    expect(statProvider("big-pickle", "", "makcode")).toBe("unknown")
  ...
  test("model aggregates prefer provider.model and use normalized model", () => {
    ...
    expect(
      toModelAggregate({ ...aggregate("big-pickle", "makcode"), provider_model: "claude-sonnet-4-5" }),
    ).toMatchObject([
      {
        provider: "anthropic",
  ...
  test("provider aggregates never keep makcode as the provider", () => {
    expect(toProviderAggregate({ ...aggregate("big-pickle", "makcode"), provider_model: "gpt-5" })).toMatchObject([
      { provider: "openai" },
    ])
    expect(toProviderAggregate(aggregate("big-pickle", "makcode"))).toMatchObject([{ provider: "unknown" }])
  })

  test("geo aggregates never keep makcode or big-pickle dimensions", () => {
    expect(toGeoAggregate({ ...aggregate("big-pickle", "makcode"), country: "US" })).toMatchObject([
