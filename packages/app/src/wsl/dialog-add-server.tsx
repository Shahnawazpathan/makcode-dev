...
import { showToast } from "@makcode-ai/ui/toast"
...
type WslServerStep = "wsl" | "distro" | "makcode"

const STEPS: WslServerStep[] = ["wsl", "distro", "makcode"]
...
export function DialogAddWslServer(props: DialogWslServerProps = {}) {
  ...
  const makcodeCheck = createMemo(() => {
    const distro = selectedDistro()
    if (!distro) return null
    return current()?.makcodeChecks[distro] ?? null
  })
  ...
  const makcodeReady = createMemo(() => {
    const check = makcodeCheck()
  ...
  const installingOpencode = createMemo(() => {
    ...
    return job?.kind === "install-makcode" && job.distro === selectedDistro()
  })
  const allReady = createMemo(() => wslReady() && distroReady() && makcodeReady())
  const addDisabled = createMemo(() => {
    const job = current()?.job
    if (!job) return store.adding
    return store.adding || job.kind !== "probe-makcode"
  })
  ...
  const autoProbe = createMemo(() => {
    ...
    if (!state.makcodeChecks[distro]) {
      return { key: `probe-makcode:${distro}`, run: () => api.probeOpencode(distro) }
    }
    return null
  })
  ...
  const makcodeMessage = createMemo(() => {
    ...
    if (makcodeReady()) {
      return distro
        ? language.t("wsl.onboarding.makcodeReadyIn", { distro })
        : language.t("wsl.onboarding.makcodeReady")
    }
    return distro
        ...
        title:
          step === "wsl"
            ...
            : step === "distro"
              ...
              : language.t("wsl.onboarding.step.makcode"),
        state:
          active === step
            ? "current"
            : step === "wsl"
              ...
              : step === "distro"
                ? distroReady()
                  ...
                  : index > activeIndex
                    ? "locked"
                    : "warning"
                : makcodeCheck()?.matchesDesktop === false
                  ? "warning"
                  : makcodeReady()
                    ? "done"
                    : index > activeIndex
      ...
      >
        ...
        >
          ...
          <Switch>
            ...
            <Match when={activeStep() === "makcode"}>
              <div class="rounded-md bg-surface-base p-4 flex flex-col gap-3">
                <div class="flex items-center justify-between gap-3">
                  <div class="text-14-medium text-text-strong">{language.t("wsl.onboarding.step.makcode")}</div>
                  <div class="flex items-center gap-2">
                    <Show when={selectedDistro()}>
                      <Button
                        variant="ghost"
                    ...
                    <Show when={!makcodeReady() || makcodeCheck()?.matchesDesktop === false}>
                      <Button
                        variant="secondary"
                        size="large"
                        disabled={busy()}
                ...
                <div class="text-12-regular text-text-weak whitespace-pre-wrap break-words">{makcodeMessage()}</div>
                <Show when={makcodeCheck()?.matchesDesktop === false ? makcodeCheck() : null}>
                  {(check) => (
                    <div class="rounded-md border border-border-weak-base px-3 py-3 flex flex-col gap-1">
                      <div class="text-12-regular text-text-weak">
          ...
          <Show when={activeStep() === "makcode" && allReady() && selectedDistro()}>
