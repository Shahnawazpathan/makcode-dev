  ...
  test("moves an existing canonical web bucket into local scope", () => {
    expect(
      migrateCanonicalLocalServerState(
        {
          list: [],
          projects: { "https://makcode.example.com": [{ worktree: "/remote", expanded: true }] },
          lastProject: { "https://makcode.example.com": "/remote" },
        },
        ServerConnection.Key.make("https://makcode.example.com"),
      ),
    ).toEqual({
      list: [],
      ...
      migrateCanonicalLocalServerState(
        {
          projects: {
            ...
            "https://makcode.example.com": [
              { worktree: "/local", expanded: true },
              { worktree: "/remote", expanded: true },
            ],
          },
          lastProject: { local: "/local", "https://makcode.example.com": "/remote" },
        },
        ServerConnection.Key.make("https://makcode.example.com"),
      ),
    ).toEqual({
