...
import { createOpencode, type ToolPart } from "@makcode-ai/sdk"
...
console.log("🚀 Starting makcode server...")
const makcode = await createOpencode({
  port: 0,
})
...
void (async () => {
  const events = await makcode.client.event.subscribe()
...
app.message(async ({ message, say }) => {
  ...
  if (!session) {
    console.log("🆕 Creating new makcode session...")
    const { client, server } = makcode
    ...
    console.log("✅ Created makcode session:", createResult.data.id)
  ...
  console.log("📝 Sending to makcode:", message.text)
