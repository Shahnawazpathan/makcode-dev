...
import { LocationServiceMap } from "@makcode-ai/core/location-service-map"
import { SessionExecutionLocal } from "@makcode-ai/core/session/execution/local"
...
export function createRoutes(password?: string) {
  return makeRoutes(
    password
      ? ServerAuth.Config.configLayer({ username: "makcode", password: Option.some(password) })
      : ServerAuth.Config.layer,
  )
}

export function createEmbeddedRoutes() {
  return makeRoutes(ServerAuth.Config.configLayer({ username: "makcode", password: Option.none() }))
}
