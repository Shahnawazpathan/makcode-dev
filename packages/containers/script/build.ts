...
const reg = process.env.REGISTRY ?? "ghcr.io/shahnawaz-pathan"
...
const setup = async () => {
  ...
  if (list.includes("makcode")) {
    await $`docker buildx use makcode`
    return
  }
  await $`docker buildx create --name makcode --use`
}
