...
import type { IntegrationOAuthMethodRegistration } from "@makcode-ai/plugin/v2/effect/integration"
import { define } from "@makcode-ai/plugin/v2/effect/plugin"
    ...
    yield* ctx.catalog.transform((catalog) => {
      for (const [providerID, item] of Object.entries(providers ?? {})) {
        catalog.provider.update(providerID, (provider) => {
          ...
          if (item.name !== undefined) provider.name = item.name.replaceAll("OpenCode", "MakCode")
