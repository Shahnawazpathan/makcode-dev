...
`"$schema": "https://makcode.ai/config.json"` so the user's editor catches
...
```ts
import type { Plugin } from "@makcode-ai/plugin"
...
- `OPENCODE_CONFIG_CONTENT='{"$schema":"https://makcode.ai/config.json"}'`:
...
- Validate against the schema before writing. If you are unsure of a field's
  ...
  `https://makcode.ai/config.json` and read the schema rather than guessing.
