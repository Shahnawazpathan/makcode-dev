...
import { ConfigProvider } from "@makcode-ai/core/config/provider"
import { AppNodeBuilder } from "@makcode-ai/core/effect/app-node-builder"
import { LayerNode } from "@makcode-ai/core/effect/layer-node"
import { ConfigMigrateV1 } from "@makcode-ai/core/v1/config/migrate"
...
import { Policy } from "@makcode-ai/core/policy"
...
describe("Config", () => {
  ...
  it.live("loads makcode JSON and JSONC files from lowest to highest priority", () =>
    Effect.acquireRelease(
      Effect.promise(() => tmpdir()),
      (tmp) => Effect.promise(() => tmp[Symbol.asyncDispose]()),
    ).pipe(
      Effect.flatMap((tmp) =>
        Effect.gen(function* () {
          yield* Effect.promise(() =>
            Promise.all([
              fs.writeFile(
                path.join(tmp.path, "makcode.json"),
                JSON.stringify({ $schema: "base", providers: { base: provider } }),
              ),
              fs.writeFile(
                path.join(tmp.path, "makcode.jsonc"),
                `{
          ...
          return yield* Effect.gen(function* () {
            ...
            expect(documents[0]?.path).toBe(path.join(tmp.path, "makcode.json"))
            expect(documents[1]?.info.providers?.last).toBeInstanceOf(ConfigProvider.Info)

            yield* Effect.promise(() =>
              fs.writeFile(path.join(tmp.path, "makcode.jsonc"), JSON.stringify({ $schema: "changed" })),
            )
            expect(
    ...
    Effect.acquireRelease(
      ...
      Effect.flatMap((tmp) =>
        Effect.gen(function* () {
          yield* Effect.promise(() =>
            Promise.all([
              fs.writeFile(path.join(tmp.path, "makcode.json"), JSON.stringify({ $schema: "base" })),
              fs.writeFile(path.join(tmp.path, "makcode.jsonc"), "{ invalid"),
            ]),
          )
          return yield* Effect.gen(function* () {
            const config = yield* Config.Service
  ...
  it.live("loads global, ancestor, and .makcode configuration up to the project boundary", () =>
    Effect.acquireRelease(
      Effect.promise(() => tmpdir()),
      (tmp) => Effect.promise(() => tmp[Symbol.asyncDispose]()),
    ).pipe(
      Effect.flatMap((tmp) => {
        ...
        return Effect.gen(function* () {
          yield* Effect.promise(async () => {
            ...
            await fs.mkdir(path.join(root, ".makcode"), { recursive: true })
            await fs.mkdir(path.join(directory, ".makcode"), { recursive: true })
            await Promise.all([
              fs.writeFile(path.join(tmp.path, "makcode.json"), JSON.stringify({ $schema: "outside" })),
              fs.writeFile(path.join(global, "makcode.json"), JSON.stringify({ $schema: "global" })),
              fs.writeFile(path.join(root, "makcode.json"), JSON.stringify({ $schema: "root" })),
              fs.writeFile(path.join(parent, "makcode.jsonc"), JSON.stringify({ $schema: "parent" })),
              fs.writeFile(path.join(directory, "makcode.json"), JSON.stringify({ $schema: "directory" })),
              fs.writeFile(path.join(root, ".makcode", "makcode.json"), JSON.stringify({ $schema: "root-dot" })),
              fs.writeFile(
                path.join(directory, ".makcode", "makcode.jsonc"),
          ...
          return yield* Effect.gen(function* () {
            ...
            expect(entries.filter((entry) => entry.type === "directory").map((entry) => entry.path)).toEqual([
              ...
              AbsolutePath.make(path.join(root, ".makcode")),
              AbsolutePath.make(path.join(directory, ".makcode")),
            ])
