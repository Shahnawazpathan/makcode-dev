# MakCode V2 Promise Plugin API

The Promise plugin API is the async/await equivalent of `@makcode-ai/plugin/v2/effect`. It grants plugins the same two in-process capabilities:

- `hook` installs behavior at an MakCode extension point.
...
```ts
import { define } from "@makcode-ai/plugin/v2/promise"
