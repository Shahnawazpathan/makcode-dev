...
export async function spawnWslSidecar(
  ...
  const makcode = await resolveWslOpencode(distro)
  if (!makcode) throw new Error(`MakCode is not installed in ${distro}`)

  const port = await allocatePort()
  const password = randomUUID()
  const username = "makcode"
  const script = [
    "set -euo pipefail",
    'cd "$HOME" || cd /',
    ...
    `exec ${shellEscape(makcode)} --print-logs --log-level ${app.isPackaged ? "WARN" : "INFO"} serve --hostname 0.0.0.0 --port ${port}`,
  ].join("\n")
