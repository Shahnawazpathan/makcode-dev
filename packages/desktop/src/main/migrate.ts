...
function tauriAppId() {
  return app.isPackaged ? TAURI_APP_IDS[CHANNEL] : "ai.makcode.desktop.dev"
}

// Migrate a single Tauri .dat file into the corresponding electron-store.
// `makcode.settings.dat` is special: it maps to the `makcode.settings` store
...
// renderer already passes via IPC (e.g. `"default.dat"`, `"makcode.global.dat"`).
function migrateFile(datPath: string, filename: string) {
  ...
  // makcode.settings.dat → the electron settings store ("makcode.settings").
  // All other .dat files keep their full filename as the store name so they match
  // what the renderer passes via IPC (e.g. "default.dat", "makcode.global.dat").
  const storeName = filename === "makcode.settings.dat" ? "makcode.settings" : filename
