...
Practical reference for a future `packages/server` split after the makcode
server moved to the Effect HttpApi backend.

## Current State

- The server still lives in `packages/makcode`.
...
Until enough shared service code lives outside `packages/makcode`, a future
`packages/server` should either:

- own pure HttpApi contracts only, or
- accept host-provided services/layers/callbacks from `packages/makcode`

It should not import `packages/makcode` services while `packages/makcode`
...
2. Move stable domain schemas into shared packages only when they no longer
   depend on makcode-local runtime modules.
3. Extract pure HttpApi contract modules into `packages/server` once the contract
   can compile without importing `packages/makcode` implementation details.
