...
Trim `packages/makcode/src/server/routes/instance/httpapi/public.ts` until OpenAPI generation is mostly a direct projection of the `HttpApi` route declarations, without breaking the generated SDK surface.
...
- `[x]` Add or extend `packages/makcode/test/server/httpapi-query-schema-drift.test.ts`.
...
- `[x]` `bun test --timeout 5000 test/server/httpapi-query-schema-drift.test.ts` from `packages/makcode`.
- `[x]` `bun typecheck` from `packages/makcode`.

### PR 2: Delete Spec-Only Workspace Query Injection

- `[x]` Edit `packages/makcode/src/server/routes/instance/httpapi/public.ts`.
...
- `[x]` `bun test --timeout 5000 test/server/httpapi-query-schema-drift.test.ts` from `packages/makcode`.
- `[x]` `bun dev generate > /tmp/makcode-openapi.json` from `packages/makcode`.
...
- Edit `packages/makcode/src/server/routes/instance/httpapi/public.ts`.
...
- `bun dev generate > /tmp/makcode-openapi.json` from `packages/makcode`.
...
- Check source schemas in files like `packages/makcode/src/session/schema.ts`, `packages/makcode/src/permission/schema.ts`, and pty schema definitions.
...
- Edit route group files under `packages/makcode/src/server/routes/instance/httpapi/groups/`.
- Replace SDK-visible `HttpApiError.BadRequest` / `HttpApiError.NotFound` with explicit error schemas from `packages/makcode/src/server/routes/instance/httpapi/errors.ts` or add new ones there.
