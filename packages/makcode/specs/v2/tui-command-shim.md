...
Delete `packages/makcode/src/cli/cmd/tui/plugin/command-shim.ts`.

In `packages/makcode/src/cli/cmd/tui/plugin/api.tsx`, remove:

- the `createCommandShim` import
- the `command: createCommandShim(...)` field from `createTuiApi(...)`

In `packages/makcode/src/cli/cmd/tui/plugin/runtime.ts`, remove:
...
- `bun typecheck` in `packages/makcode`
- TUI plugin loader tests in `packages/makcode` if runtime plugin API wiring changed
