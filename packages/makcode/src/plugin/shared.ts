...
export const DEPRECATED_PLUGIN_PACKAGES = ["makcode-openai-codex-auth", "makcode-copilot-auth"]
...
export async function checkPluginCompatibility(target: string, makcodeVersion: string, pkg?: PluginPackage) {
  if (!semver.valid(makcodeVersion) || semver.major(makcodeVersion) === 0) return
  const hit = pkg ?? (await readPluginPackage(target).catch(() => undefined))
  if (!hit) return
  const engines = hit.json.engines
  if (!isRecord(engines)) return
  const range = engines.makcode
  if (typeof range !== "string") return
  if (!semver.satisfies(makcodeVersion, range)) {
    throw new Error(`Plugin requires makcode ${range} but running ${makcodeVersion}`)
  }
}
