...
import { MakCodeHttpApi } from "./api"
...
const LegacyComponentDescriptions: Record<string, string> = {
  LogLevel: "Log level",
  ServerConfig: "Server configuration for makcode serve and web commands",
...
function fixSelfReferencingComponents(spec: OpenApiSpec) {
  ...
  const raw: OpenApiSpec = OpenApi.fromApi(MakCodeHttpApi)
...
export const PublicApi = MakCodeHttpApi.annotateMerge(
  OpenApi.annotations({
    title: "makcode",
    version: "1.0.0",
    description: "makcode api",
