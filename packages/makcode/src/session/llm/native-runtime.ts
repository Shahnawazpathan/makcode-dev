...
import type { LLMClientShape } from "@makcode-ai/llm/route"
...
function statusWithFetch(
  ...
  if (providerID !== "openai" && providerID !== "anthropic" && !providerID.startsWith("makcode"))
    return { type: "unsupported", reason: "provider is not openai, makcode, or anthropic" }
...
export function stream(input: StreamInput): StreamResult {
  ...
  // Integration point with @makcode-ai/llm: native-request lowers session data
...
export function nativeTools(tools: Record<string, Tool>, input: Pick<StreamInput, "messages" | "abort">) {
  return Object.fromEntries(
    Object.entries(tools).map(([name, item]) => [
      name,
      // Tool execution remains makcode-owned. The native runtime only adapts
      // the @makcode-ai/llm tool call back into the AI SDK Tool.execute shape.
      NativeTool.make({
