...
describe("makcode read-only commands (smoke)", () => {
  ...
  cliIt.live(
    ...
    ({ makcode }) =>
      Effect.gen(function* () {
        const r = yield* makcode.spawn(["mcp", "list"])
        makcode.expectExit(r, 0, "mcp list")
      }),
    60_000,
  )
  ...
  cliIt.live(
    ...
    ({ makcode }) =>
      Effect.gen(function* () {
        const r = yield* makcode.spawn(["providers", "list"])
        makcode.expectExit(r, 0, "providers list")
  ...
  cliIt.live(
    ...
    ({ makcode }) =>
      Effect.gen(function* () {
        const r = yield* makcode.spawn(["models"])
        makcode.expectExit(r, 0, "models")
  ...
  cliIt.live(
    ...
    ({ makcode }) =>
      Effect.gen(function* () {
        const r = yield* makcode.spawn(["agent", "list"])
        makcode.expectExit(r, 0, "agent list")
      }),
    60_000,
  )
  ...
  cliIt.live(
    ...
    ({ makcode }) =>
      Effect.gen(function* () {
        const r = yield* makcode.spawn(["session", "list"])
        makcode.expectExit(r, 0, "session list")
      }),
    60_000,
  )
  ...
  cliIt.live(
    ...
    ({ makcode }) =>
      Effect.gen(function* () {
        const r = yield* makcode.spawn(["stats"])
        makcode.expectExit(r, 0, "stats")
      }),
    60_000,
  )
  ...
  cliIt.live(
    ...
    ({ makcode }) =>
      Effect.gen(function* () {
        const r = yield* makcode.spawn(["db", "path"])
        makcode.expectExit(r, 0, "db path")
