...
// terminal sizes. The default makcode tui command is excluded —
// `makcode --help` includes an ASCII banner that pulls in the install
...
// Top-level commands. Order matches what `makcode --help` prints today;
...
// top-level help on `--help` and exits 1; not a real makcode command.
const TOP_LEVEL = [
  "acp",
  "mcp",
  "attach",
...
describe("makcode CLI help-text snapshots", () => {
  ...
  cliIt.live(
    ...
    ({ makcode }) =>
      Effect.gen(function* () {
        const topLevel = yield* makcode.spawn(["--help"], { env: SNAPSHOT_ENV })
        ...
        const [failures, results] = yield* Effect.partition(
          ...
          (argv) =>
            Effect.gen(function* () {
              const result = yield* makcode.spawn([...argv, "--help"], { env: SNAPSHOT_ENV })
              if (result.exitCode !== 0) {
                return yield* Effect.fail(`makcode ${argv.join(" ")}: exit ${result.exitCode}`)
              }
              return { argv, result }
            }),
          { concurrency: 8 },
        ...
        for (const { argv, result } of results) {
          ...
          expect(normalize(result.stderr)).toMatchSnapshot(`makcode ${argv.join(" ")} --help`)
        }
