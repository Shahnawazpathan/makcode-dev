// Subprocess integration tests for `makcode run` (non-interactive mode).
// These exercise the real CLI binary against a TestLLMServer running in the
// same process. See `test/lib/cli-process.ts` for the harness — each test uses
// `makcode.run(message, opts?)` to spawn `bun src/index.ts run ...` with
...
describe("makcode run (non-interactive subprocess)", () => {
  ...
  cliIt.concurrent(
    ...
    ({ llm, makcode }) =>
      Effect.gen(function* () {
        ...
        const result = yield* makcode.run("say hi")
        makcode.expectExit(result, 0)
  ...
  cliIt.concurrent(
    ...
    ({ llm, makcode }) =>
      Effect.gen(function* () {
        ...
        const result = yield* makcode.run("use a tool", {
  ...
  cliIt.concurrent(
    ...
    ({ llm, makcode }) =>
      Effect.gen(function* () {
        ...
        const thinking = yield* makcode.run("think", { extraArgs: ["--thinking"] })
        makcode.expectExit(thinking, 0)
        expect(thinking.stdout).toBe("Thinking: considering\nanswer\n")

        yield* llm.reason("hidden", { text: "visible" })
        const plain = yield* makcode.run("think again")
        makcode.expectExit(plain, 0)
  ...
  cliIt.concurrent(
    ...
    ({ llm, makcode }) =>
      Effect.gen(function* () {
        ...
        const result = yield* makcode.run("trigger midstream error", { timeoutMs: 30_000 })
  ...
  cliIt.concurrent(
    ...
    ({ llm, makcode }) =>
      Effect.gen(function* () {
        ...
        const result = yield* makcode.run("say hi", { format: "json" })
        makcode.expectExit(result, 0)

        const events = makcode.parseJsonEvents(result.stdout)
  ...
  cliIt.concurrent(
    ...
    ({ makcode }) =>
      Effect.gen(function* () {
        const result = yield* makcode.run("use an unknown model", {
          model: "test/nonexistent-model",
          format: "json",
        })
  ...
  cliIt.concurrent(
    ...
    ({ llm, makcode }) =>
      Effect.gen(function* () {
        ...
        const result = yield* makcode.run("exercise json records", {
          format: "json",
  ...
  cliIt.concurrent(
    ...
    ({ llm, makcode }) =>
      Effect.gen(function* () {
        ...
        const result = yield* makcode.run("fail after output", { format: "json" })
  ...
  cliIt.concurrent(
    ...
    ({ home, llm, makcode }) =>
      Effect.gen(function* () {
        ...
        const denied = yield* makcode.run("request permission", { permission: { bash: "ask" } })
        makcode.expectExit(denied, 0)
        ...
        const allowed = yield* makcode.run("request permission", {
          permission: { bash: "ask" },
          extraArgs: ["--dangerously-skip-permissions"],
        })
        makcode.expectExit(allowed, 0)
        ...
        const explicitlyDenied = yield* makcode.run("request denied permission", {
          permission: { bash: "deny" },
          extraArgs: ["--dangerously-skip-permissions"],
        })
        makcode.expectExit(explicitlyDenied, 0)
  ...
  cliIt.live(
    ...
    ({ home, llm, makcode }) =>
      Effect.gen(function* () {
        ...
        const server = yield* makcode.serve()

        const result = yield* makcode.run("read the attachment", {
  ...
  cliIt.concurrent(
    ...
    ({ home, makcode }) =>
      Effect.gen(function* () {
        const result = yield* makcode.run("read the directory", {
  ...
  cliIt.live(
    ...
    ({ llm, makcode }) =>
      Effect.gen(function* () {
        yield* llm.hang
        const run = yield* makcode.startRun("wait forever")
        yield* llm.wait(1)
        run.interrupt()
