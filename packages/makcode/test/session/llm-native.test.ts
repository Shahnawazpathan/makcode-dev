...
import { LLMEvent, ToolFailure } from "@makcode-ai/llm"
import { LLMClient, RequestExecutor, WebSocketExecutor, type LLMClientShape } from "@makcode-ai/llm/route"
...
describe("session.llm-native.request", () => {
  ...
  test("only enables native runtime for supported OpenAI API-key models", () => {
    ...
    ).toEqual({ type: "unsupported", reason: "provider is not openai, makcode, or anthropic" })
    expect(
      LLMNativeRuntime.status({
        model: baseModel,
