// Subprocess test harness for the makcode CLI. Spawns the real binary against
...
//
// Configuration flows through makcode's built-in test affordances:
//   - OPENCODE_CONFIG_CONTENT      : provider config inline, no files to find
//   - OPENCODE_TEST_HOME           : pins os.homedir() → tmpdir
//   - OPENCODE_DISABLE_PROJECT_CONFIG : skip walking up for makcode.json
...
//
// Today only `makcode.run` is fully wired. The shape supports adding more
// builders (`makcode.serve(opts)`, `makcode.acp(opts)`, `makcode.auth(...)`)
...
const makcodeRoot = path.resolve(import.meta.dir, "../../")
const cliEntry = path.join(makcodeRoot, "src/index.ts")
...
// Typed equivalent of constructing argv for `makcode run`. New flags should
...
// `makcode serve` is a long-lived process — it never exits on its own.
...
// `makcode acp` speaks newline-delimited JSON-RPC over stdin/stdout. It is
...
export type OpencodeCli = {
  ...
  // Spawn `makcode serve` and wait until it's listening. Long-lived: the
  ...
  // Spawn `makcode acp` and return a duplex JSON-RPC handle. Long-lived:
...
export function withCliFixture<A, E>(
  ...
  return Effect.gen(function* () {
    ...
    const spawn = Effect.fn("makcode.spawn")(function* (args: string[], opts?: SpawnOpts) {
    ...
    const startRun = Effect.fn("makcode.startRun")(function* (message: string, opts?: RunOpts) {
      const start = Date.now()
    ...
    const serve = Effect.fn("makcode.serve")(function* (opts?: ServeOpts) {
      const argv = ["serve"]
      ...
      //   "makcode server listening on http://<host>:<port>"
      ...
      const match = yield* Deferred.await(readyDeferred).pipe(
        Effect.timeoutOrElse({
          ...
          orElse: () =>
            Effect.fail(
              new Error(
                `makcode serve did not become ready within ${readyTimeoutMs}ms\n` +
    ...
    const acp = Effect.fn("makcode.acp")(function* (opts?: AcpOpts) {
      const argv = ["acp"]
    ...
    const makcode: OpencodeCli = { run, startRun, serve, acp, spawn, expectExit, parseJsonEvents }

    return yield* fn({ llm, home, makcode })
...
function expectExit(result: RunResult, expected: number, label = "makcode") {
...
// every call site. Use this for any test that needs the makcode CLI fixture.
//
...
// resources (e.g. `makcode.serve`) without an extra `Effect.scoped` wrapper —
