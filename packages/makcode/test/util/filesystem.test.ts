...
describe("filesystem", () => {
  ...
  describe("findUp()", () => {
    ...
    test("rootFirst preserves json then jsonc order per directory", async () => {
      ...
      await fs.writeFile(path.join(tmp.path, "makcode.json"), "{}", "utf-8")
      await fs.writeFile(path.join(tmp.path, "makcode.jsonc"), "{}", "utf-8")
      await fs.writeFile(path.join(project, "makcode.json"), "{}", "utf-8")
      await fs.writeFile(path.join(project, "makcode.jsonc"), "{}", "utf-8")

      const result = await Filesystem.findUp(["makcode.json", "makcode.jsonc"], nested, tmp.path, {
        rootFirst: true,
      })
      ...
      expect(result).toEqual([
        ...
        path.join(project, "makcode.json"),
        path.join(project, "makcode.jsonc"),
      ])
    })
  })
